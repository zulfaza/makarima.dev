...
function parseCodeMeta(
  ...
  context: string
): Pick<Extract<ContentBlock, { readonly kind: "code" }>, "caption" | "title"> {
...
function parseImageBlock(
  node: ImageNode
): Extract<ContentBlock, { readonly kind: "image" }> {
  return {
    kind: "image",
    src: node.url,
    alt: node.alt ?? "",
  ...
  for (const child of tree.children) {
    switch (child.type) {
      case "paragraph":
        ...
        break
      case "code":
        blocks.push(parseCodeBlock(child, context))
        break
      default:
        fail(
          `${context}${lineSuffix(child)}`,
          `Unsupported markdown node "${child.type}"`
        )
    }
  }
